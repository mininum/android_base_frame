...
class CalligraphyFactory
{
    ...
    protected static int[] getStyleForTextView(TextView view)
    {
        ...
        if (styleIds[0] == -1)
        {
            ...
            styleIds[0] = CalligraphyConfig.get().getClassStyles().containsKey(view.getClass())
                    ? CalligraphyConfig.get().getClassStyles().get(view.getClass()) : android.R.attr.textAppearance;
        }
        return styleIds;
    }
    ...
    /**
     * Use to match a view against a potential view id. Such as ActionBar symbol etc.
     * 
     * @param view
     *            not null view you want to see has resource matching name.
     * @param matches
     *            not null resource name to match against. Its not case sensitive.
    ...
    void onViewCreatedInternal(View view, final Context context, AttributeSet attrs)
    {
        if (view instanceof TextView)
        {
            ...
            if (TextUtils.isEmpty(textViewFont))
            {
                ...
                else textViewFont = CalligraphyUtils.pullFontPathFromTheme(context, styleForTextView[0], mAttributeId);
            }
