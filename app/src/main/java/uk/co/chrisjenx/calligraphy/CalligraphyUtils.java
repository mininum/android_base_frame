...
public final class CalligraphyUtils
{
    ...
    /** 引用外部字体时，是否取消文字上下的留白 */
    public static boolean textViewIncludeFontPadding = false;
    ...
    /**
     * Applies a Typeface to a TextView. Defaults to false for deferring, if you are having issues with the textview keeping the custom Typeface, use {@link #applyFontToTextView(TextView, Typeface, boolean)}
     *
     * @param textView
     *            Not null, TextView or child of.
     * @param typeface
     *            Not null, Typeface to apply to the TextView.
    ...
    /**
     * Applies a Typeface to a TextView, if deferred,its recommend you don't call this multiple times, as this adds a TextWatcher. Deferring should really only be used on tricky views which get Typeface set by the system at weird times.
     *
     * @param textView
     *            Not null, TextView or child of.
     * @param typeface
     *            Not null, Typeface to apply to the TextView.
     * @param deferred
     *            If true we use Typefaces and TextChange listener to make sure font is always applied, but this sometimes conflicts with other {@link Spannable}'s.
     * @return true if applied otherwise false.
    ...
    public static boolean applyFontToTextView(final TextView textView, final Typeface typeface, boolean deferred)
    {
        ...
        try
        {
            // cccmax 去掉文字上下的留白 ， 所有字体自带的留白全部消失
            textView.setIncludeFontPadding(textViewIncludeFontPadding);

            float size = textView.getTextSize();// 字号
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.JELLY_BEAN)
            {
                float linespacing = 0;// 已经设置的行间距
                linespacing = textView.getLineSpacingExtra();
                float newsize = size * 0.25F + linespacing;// 补充字体上下留白，不设置行间距的时候 也会有带上默认距离
                textView.setLineSpacing(newsize, textView.getLineSpacingMultiplier());
            }

            // 给字体上部和下部增加一部分padding 不然baseline以外的字符显示不全
            int ptop = textView.getPaddingTop();
            ptop = ptop != 0 ? ptop : (int) (size * 0.15F);
            int pbottom = textView.getPaddingBottom();
            pbottom = pbottom != 0 ? pbottom : (int) (size * 0.15F);
            textView.setPadding(textView.getPaddingLeft(), ptop, textView.getPaddingRight(), pbottom);
    ...
    /**
     * Useful for manually fonts to a TextView. Will not default back to font set in {@link uk.co.chrisjenx.calligraphy.CalligraphyConfig}
     * 
     * @param context
     *            Context
     * @param textView
    ...
    static boolean applyFontToTextView(final Context context, final TextView textView, final String filePath,
            boolean deferred)
    {
     ...
     *            context
     * @param textView
     *            textView to apply to.
     * @param config
     *            Default Config
     * @param textViewFont
     *            nullable, will use Default Config if null or fails to find the defined font.
     */
    public static void applyFontToTextView(final Context context, final TextView textView,
            final CalligraphyConfig config, final String textViewFont)
    {
        applyFontToTextView(context, textView, config, textViewFont, false);
    }

    static void applyFontToTextView(final Context context, final TextView textView, final CalligraphyConfig config,
            final String textViewFont, boolean deferred)
    {
        if (context == null || textView == null || config == null)
            return;
    ...
    static String pullFontPathFromTextAppearance(final Context context, AttributeSet attrs, int attributeId)
    {
        ...
        final TypedArray typedArrayAttr = context.obtainStyledAttributes(attrs, new int[]
        ...
        final TypedArray textAppearanceAttrs = context.obtainStyledAttributes(textAppearanceId, new int[]
        { attributeId });
    ...
    static String pullFontPathFromTheme(Context context, int styleAttrId, int attributeId)
    {
        ...
        final TypedArray typedArray = theme.obtainStyledAttributes(value.resourceId, new int[]
        { attributeId });
        try
        {
     ...
     * @param styleAttrId
     *            Theme style id
     * @param subStyleAttrId
     *            the sub style from the theme to look up after the first style
     * @param attributeId
     *            if -1 returns null.
     * @return null if no theme or attribute defined.
     */
    static String pullFontPathFromTheme(Context context, int styleAttrId, int subStyleAttrId, int attributeId)
    {
        ...
        theme.resolveAttribute(styleAttrId, value, true);
        int subStyleResId = -1;
        final TypedArray parentTypedArray = theme.obtainStyledAttributes(value.resourceId, new int[]
        { subStyleAttrId });
        try
        {
        ...
        if (subStyleResId == -1)
            return null;
        final TypedArray subTypedArray = context.obtainStyledAttributes(subStyleResId, new int[]
        { attributeId });
