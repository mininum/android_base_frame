...
public class WheelView extends View
{
    ...
    /** 焦点以外的其他item的透明度 */
    private float alpha_min = 0.55F;
    /** 焦点以外的其他item的缩放 */
    private float scale_min = 0.8F;
    /** 焦点以外其他Item向中心item的最大缩进比例 */
    private float indent_max = 0.5F;
    /** x轴旋转最大角度 */
    private float rotationX_max = 45;
    ...
    /**
     * Gets mtn of visible items
     * 
     * @return the mtn of visible items
     */
    ...
    /**
     * Sets the desired mtn of visible items. Actual amount of visible items depends on wheel layout parameters. To apply changes and rebuild view call measure().
     * 
     * @param count
     *            the desired mtn for visible items
     */
    ...
    /** 设置边缘item最小透明度 */
    public void setItemAlphaMin(float alphaMin)
    {
        this.alpha_min = alphaMin;
    }

    /** 设置边缘item最小缩放比例 */
    public void setItemScaleMin(float scaleMin)
    {
        this.scale_min = scaleMin;
    }

    /** 设置上下item 向中心缩进 0～1 */
    public void setItemIndentMax(float indent)
    {
        this.indent_max = indent;
    }

    /** 设置item透视倾斜的最大角度 */
    public void setItemRotationXMax(float rotationX)
    {
        this.rotationX_max = rotationX;
    }
    ...
    private void drawItems(Canvas canvas)
    {
        ...
        int iten_h = getItemHeight();
        int top = (currentItem - firstItem) * iten_h + (iten_h - getHeight()) / 2;
        int offset = -top + scrollingOffset;
        canvas.translate(PADDING, offset);

        try
        {
            // 这个代码块 负责 item的透明变化和缩放变化
            int h_half = getMeasuredHeight() / 2;// 控件中心高度
            for (int i = 0; i < itemsLayout.getChildCount(); i++)
            {
                View child = itemsLayout.getChildAt(i);
                int child_c = (int) ((i + 0.5F) * iten_h + offset);// 每个itme中心位置
                int center_offsize = h_half - child_c;// item中心与整体中心的高度偏差
                int ofs = Math.abs(center_offsize);
                float alpha = alpha_min;
                float scale = scale_min;
                float indent = 0;
                float rotationX = 0;

                int transform_range = iten_h * 2;// 变形范围
                // 变形范围内
                if (ofs < transform_range)
                {
                    float x = 1 - 1.0F * ofs / transform_range;// 距离中间比例
                    alpha = alpha_min + x * (1 - alpha_min);// 偏移比例的透明度
                    scale = scale_min + x * (1 - scale_min);// 偏移比例的缩放
                }

                float xx = 1.0F * center_offsize / h_half;// 距离中间比例
                int direction = xx > 0 ? 1 : -1;// 方向
                xx = Math.abs(xx);
                if (indent_max > 0) // 上下缩进
                {
                    indent = direction * h_half * indent_max * xx * (xx / 2);
                }
                if (rotationX_max > 0)// x轴旋转 透视倾斜
                {
                    rotationX = direction * rotationX_max * xx;
                }
    ...
    private void buildViewForMeasuring()
    {
        ...
        {
            // 只先new一个就好了··用来计算宽高 主要作用是大幅度提升初始速度
            addViewItem(0, true);
            firstItem = 0;
        }
    }
