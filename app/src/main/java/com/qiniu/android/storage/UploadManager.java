...
/**
 * 七牛文件上传管理器 一般默认可以使用这个类的方法来上传数据和文件。这个类自动检测文件的大小， 只要超过了{@link Configuration#putThreshold}
 */
public final class UploadManager
{
    ...
    {
        this.config = config;
        this.client = new Client(config.proxy, config.connectTimeout, config.responseTimeout, config.urlConverter,
                config.dns);
    }
            ...
            final UploadOptions options)
    {
        ...
        ResumeUploader uploader = new ResumeUploader(client, config, file, key, decodedToken, completionHandler,
                options, recorderKey);
