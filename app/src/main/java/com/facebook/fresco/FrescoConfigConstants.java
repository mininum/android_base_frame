...
public class FrescoConfigConstants
{
    ...
    // + File.separator + RT.defaultImage + File.separator;
    ...
    // 图片显示
    public static ImageRequest getImageRequest(SimpleDraweeView view, String uri, Point imgSize)
    {
        ...
        // 调整大小
        if (imgSize != null && imgSize.x > 0 && imgSize.y > 0)
        {
            // 使用预制尺寸
            builder.setResizeOptions(new ResizeOptions(imgSize.x, imgSize.y));
        } else if (view != null && view.getLayoutParams().width > 0 && view.getLayoutParams().height > 0)
        {
            // 按view尺寸 有固定的大小的时候才会调整
        ...
        if (imageRequest.getResizeOptions() != null)
        {
            if (FrescoImageHelper.log())
                DLOG.i("Fresco", "mResizeOptions=" + imageRequest.getResizeOptions().width + ","
     ...
     */
    public static DraweeController getDraweeController(ImageRequest imageRequest, boolean retry, SimpleDraweeView view,
            boolean playAnimation, BaseControllerListener controllerListener)
    {
        DraweeController draweeController = null;
        PipelineDraweeControllerBuilder builder = Fresco.newDraweeControllerBuilder();
        // .reset()//重置
        builder.setAutoPlayAnimations(playAnimation);// 自动播放图片动画
        // builder.setCallerContext(callerContext);//回调
    ...
    public static abstract class FrescoPreHandleListener extends BaseControllerListener<Object>
    {
        ...
        /** 硬件加速应该开还是关 */
        public boolean manageLayer = true;
        ...
        {
            if (FrescoImageHelper.log())
                DLOG.i("Fresco", "onSubmit");
        }

        public void onFinalImageSet(String id, Object imageInfo, Animatable animatable)
        {
            if (FrescoImageHelper.log())
                DLOG.i("Fresco", "onFinalImageSet");
            if (imageInfo != null && imageInfo instanceof ImageInfo)
            {
                try
                {
                    ...
                    // --------------------------------控制图片尺寸或者硬件加速,确保各种手机都可以显示特大图片
                    manageLayer = true;
                    if (!isGif)
                    {
                        CloseableStaticBitmap csb = (CloseableStaticBitmap) imageInfo;
                        int max = UiUtil.getMaxTextureSize(RT.application);
                        // 超出gpu尺寸
                        if (max < width || max < height)
                        {
                            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT)
                            {
                                // 不能在bitmap原对象上修改图片 ，只能修改硬件加速
                                if (draweeview != null)
                                {
                                    UiUtil.manageLayer(draweeview, false);
                                    manageLayer = false;
                                }
                            } else
                            {
                    ...
                    if (draweeview != null && manageLayer)
                    {
                        UiUtil.manageLayer(draweeview, true);
                    }
                    // ----------------------------------
                    ...
                    if (FrescoImageHelper.log())
                        DLOG.i("Fresco",
                                "FrescoPreHandleListener onFinalImageSet  w=" + imageinfo.getWidth() + " h="
                                        + imageinfo.getHeight() + " isGif=" + isGif + " ratio=" + ratio
                                        + " manageLayer=" + manageLayer + "\nview=" + draweeview);

                } catch (Exception e)
                {
                    DLOG.e("Fresco", e.getMessage());
                    e.printStackTrace();
                }
            }
        ...
        {
            if (FrescoImageHelper.log())
                DLOG.i("Fresco", "onFailure");
        }

        public void onRelease(String id)
        {
            if (FrescoImageHelper.log())
                DLOG.i("Fresco", "onRelease");
        }

        public void onIntermediateImageSet(String id, Object imageInfo)
        {
            if (FrescoImageHelper.log())
                DLOG.i("Fresco", "onIntermediateImageSet");
        }

        public void onIntermediateImageFailed(String id, Throwable throwable)
        {
            if (FrescoImageHelper.log())
                DLOG.i("Fresco", "onIntermediateImageFailed");
        }
    ...
    {
        ImageRequest ir = getImageRequest(null, url, null);
        return sImagePipelineConfig.getCacheKeyFactory().getEncodedCacheKey(ir);
    }
    ...
    {
        // 版本文件夹
        String versiondir = String.format((Locale) null, "%s.ols%d.%d", new Object[]
        { "v2", Integer.valueOf(100), Integer.valueOf(1) });
        // 图片子文件夹
    ...
    public static void autoResizeBitmap(Bitmap srcBitmap, Point needSize)
    {
        ...
        if (w > 0 && h > 0 && src_w > w && src_h > h && !srcBitmap.isRecycled())
        {
            ...
            {
                // 在原bitmap对象上进行修改
                Bitmap sampleBitmap = DMImageTool.zoomBitmap(srcBitmap, w, h);
                ...
                bb = null;
                if (FrescoImageHelper.log())
                    DLOG.i("Fresco", " autoResizeBitmap srcBitmap[" + src_w + "," + src_h + "] change to newBitmap[" + w
                            + "," + h + "]");
            } catch (Throwable e)
            {
                DLOG.i("Fresco", " autoResizeBitmap srcBitmap[" + src_w + "," + src_h + "] change to newBitmap[" + w
